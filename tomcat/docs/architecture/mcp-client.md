...
> 单一事实源：连接器抽象（`Connector` trait / `ConnectorType {Mcp, Cli, A2a}` / `ConnectorRegistry`）在 `core/connector/mod.rs`；**MCP 连接器**的运行时事实源在 `core/connector/mcp/manager.rs::McpManager`；工具暴露给模型经 `core/llm/system_prompt.rs::ToolSurface`；图片回流复用 `core/agent_loop/tool_exec/branches/read.rs` 的 `ChatMessageContentPart::InputImage` 路径。**本文描述的 `core/connector/` 模块在当前工作树尚不存在（`rg mcp` 命中的都是 serve NDJSON，不是连接器），全部为新增。**
...
开机：连上 MCP、把它的工具登记进「工具注册表」
   ...
   代码：ConnectorRegistry::new(&config) 连接 + register_tool(name=mcp__{server}__{tool},
         plugin_id="mcp:{server}", params=inputSchema)   (api/chat/context.rs · 新增)
         执行器换成 CompositeToolExecutor{plugin, mcp}      (core/connector/mod.rs · 新增；DefaultToolRegistry::new 注入)
         ConnectorRegistry 存进 GlobalServices             (api/chat/session_runtime.rs · 加字段，供生命周期/命令面)
        │
        ▼
每轮向模型提问前：工具菜单【自动】含 MCP
   人话：observe_tool_surface 本就 list_tools 整个注册表，MCP 已在里面——不用为 MCP 单独合并
   代码：observe_tool_surface() → tool_registry.list_tools()  (api/chat/run_loop/mod.rs · 无需改)
...
出口转换器：把 JSON 结果里的图片翻成模型能看的图  ← 关键、且向后兼容
   ...
   代码：extract_tool_result_media(result, files_runtime)   (tool_dispatcher.rs · 把 plugin_tool_model_text 升级)
           text 块  → ToolExecOutcome.model_text
           image 块 → follow_up_parts（复用 read.rs 图片路径：小图内联/大图 Files API）
...
> 阅读顺序（说人话）：从上往下就是一次完整调用——开机连 MCP 并把工具登记进注册表 → 工具菜单经 `list_tools` 自动含 MCP → 模型点了某个 MCP 工具 → 注册表按 `plugin_id` 路由到 `McpManager` → 出口转换器把结果里的图片翻成 `InputImage`（纯文本插件 no-op）→ 文字给模型看、图片走现成回流。**核心逻辑改两处**：`tool_dispatcher.rs`（出口转换器）、`context.rs`（建 `CompositeToolExecutor` + 注册 MCP 工具）；**不再改** `run_loop`/`system_prompt`/`tool_exec`（`list_tools` 自动上工具面）。完整清单见 §5。

### B. 状态机：单个 MCP server 连接生命周期

> 专业：磁盘只有配置，无持久连接状态；下列状态由 `McpManager` 内存中的 `ServerEntry` 推导。信任检查默认放行（config 即信任，R7），只有项目来源首见/命令变化才需一次确认。
> 说人话：你全局配的 server 配好直接连（无感）；只有「项目仓库带来的配置第一次用」或「命令被改」才需确认一次（本期 `/connector trust` 批一次，非启动时打断式弹框）；崩了有限次退避重连；关机杀子进程。
...
PreSpawn 信任检查（spawn 前唯一关卡，默认无感放行）
  ...
  │                │ 握手超时/进程退出 → Failed（记 stderr 尾，退避）
  │                Ready ──传输断裂──▶ Disconnected ──backoff(≤N)──▶ Ready（重连成功）/ 预算耗尽 → Failed
...
关机 / McpManager Drop（任意状态）→ kill_on_drop 杀子进程树
```
...
| Connecting | 握手超时 / 进程立即退出 | Failed | 记录 stderr 尾部，进退避 | 起不来就记错误，不卡住 agent。 |
| Ready | 传输断裂（EOF/管道断） | Disconnected | 先撤销 `plugin_id="mcp:{server}"` 的已注册工具，再标记失效、触发重连 | 断了先把失效工具从菜单撤下，别让模型点一个必失败的工具。 |
| Disconnected | 退避后重连成功 | Ready | 重新 `tools/list`，再重新注册工具 | 有限次退避自愈；工具可能随重连更新。 |
| Disconnected | 重连预算耗尽 | Failed | 撤销已注册工具、停止重连；**不 replay 在途调用** | 反复起不来就从菜单撤下，绝不重放一个可能已执行的调用。 |
| any | 会话关闭 / `McpManager` Drop | (terminated) | `kill_on_drop` 杀子进程树 | 关机把子进程一起带走。 |
...
| ConnectorType | 连接器类型枚举 `{Mcp, Cli, A2a}`。 | `core/connector/mod.rs::ConnectorType` | 决定工具名前缀（`mcp__`/`cli__`/`a2a__`）与连接方式；本期只 `Mcp` 有实现，`Cli`/`A2a` 预留。 | 这个连接器是哪一类。 |
...
| stdio 传输 | 用子进程的 stdin/stdout 传 换行分隔的 JSON-RPC 帧。 | `core/connector/mcp/transport.rs`（经 `rmcp` crate） | `kill_on_drop`；8MiB 单行上限；`env_clear` 后仅注入 config `env` + 最小白名单（**必含 `PATH`/`HOME`，否则找不到 `node`/`npx`、npm 缓存失效**）。 | 通过管道跟子进程一问一答，环境清干净但留下能找到 node 的必需项。 |
...
| InputImage 回流 | 把图片（base64 内联或 Files API 上传）变成模型可见 vision 内容。 | `core/llm/types.rs::ChatMessageContentPart::{image_base64_data,image_upload,image_file_id}` | 小图内联、大图上传，`upload_decision_by_size` 决定；复用 `read.rs` 同一路径。 | 「让模型真正看见图」的现成管道，直接复用。 |
| Trust 身份 | 一个 server 的信任判据，**默认 = 启动命令指纹** `hash(command+args+env+cwd)`（绑命令而非 server 名，防 MCPoison）；**可选强档**再加内容身份（npm integrity / 二进制 SHA256）。 | `core/connector/mcp/trust.rs::TrustStore`（`~/.tomcat/connector-trust.json`） | 默认 config 即信任（全局/curated/命令未变 → 无感 spawn）；**仅**项目来源首见 / 命令指纹变化 → 一次确认；用户 server 浮动版本只告警不拦（curated 钉死）。 | 记住「我放行过的启动命令长这样」；命令被换了（MCPoison）才再问一次，其余时候不打扰。 |
...
2. **图片必须回流**：本子系统存在的唯一动机是 Phase 2 的**视觉验收**；continue 丢图、pi_agent_rust 图片入 details 都会让「截图→模型看见」断链。Tomcat 的 `follow_up_parts`+`InputImage` 已经把这条管道建好（read.rs），复用即可，零新管道。
...
4. **统一进 registry + 出口转换器（Option B）**：MCP 工具像插件一样注册进 `ToolRegistry`（`plugin_id="mcp:{server}"`，`CompositeToolExecutor` 路由到 `McpManager`），`list_tools` 自动上工具面；`ToolRegistry` 契约是 JSON-only（无原生图片 parts 通道），故在 `tool_dispatcher` 出口把 `plugin_tool_model_text` 升级为 `extract_tool_result_media()`——`image` 块→`follow_up_parts`（复用 read.rs），**对纯文本插件 no-op、向后兼容**。这比给 MCP 单开 `mcp__` 前缀专线更优雅：消除可见/执行不对称与前缀抢注隐患，还顺带解锁「能返图的插件」（见 §3.1 R4）。
...
| **R4 工具面接入 + 执行路径** | MCP 工具独立走 `mcp__` 前缀分支，还是统一进 `ToolRegistry`？ | **采用 Option B**：MCP 工具**注册进 `ToolRegistry`**（`plugin_id="mcp:{server}"`，由 `CompositeToolExecutor` 按 `plugin_id` 路由到 `McpManager`）；`list_tools` **自动并入工具面**（不改 run_loop/system_prompt）；在 `tool_dispatcher` 的 registry 分支加**向后兼容的「工具结果媒体转换器」**（`content[].type=="image"`→`follow_up_parts`，对纯文本插件 no-op）。**拒绝** 独立 `mcp__` 前缀分支 | Tomcat：`core/tools/contract/registry.rs::{ToolExecutor,ToolRegistry}`（`DefaultToolRegistry` 持**单个** `executor: Arc<dyn ToolExecutor>`、`call_tool` 把输出 `wrap_tool_result` 成 `{content,details}`；JSON 边界，**签名不改**）、`tool_dispatcher.rs:313-318`（registry 分支现填 `follow_up_parts: Vec::new()`、`plugin_tool_model_text(&result)`，改成转换器）、`run_loop/mod.rs::observe_tool_surface`（`list_tools(None)`→`from_plugin_tools`，自动含 MCP）、`ext/plugin_tool_executor.rs`（executor 参照）；外部：`codex convert_mcp_content_to_items`（JSON image→input_image 转换器范式）、`pi_agent_rust` 把 MCP 挂成原生 `Tool` | **根因（查证）**：注册表/插件契约是 JSON-only（`execute -> serde_json::Value`，因插件是 rquickjs JS-VM），`follow_up_parts` 是 PR-RJ T3-c 给 builtin `read` 的**原生** 概念，registry 分支从来没这通道（填 `Vec::new()`，是「无通道」非「清空」；git `e8a5b7fc`/`2ffd58a2`）。**Option B 设计（三件）**：(1) `McpToolExecutor`(impl `ToolExecutor`) 包 `McpManager`，`CompositeToolExecutor` 按 `tool.plugin_id`（`mcp:*`→McpManager、其余→JS VM）路由；(2) `ConnectorRegistry` 连上 server 后把工具 `register_tool` 进 `DefaultToolRegistry`（`plugin_id="mcp:{server}"`，名 `mcp__{server}__{tool}`）；(3) `tool_dispatcher` registry 分支把 `plugin_tool_model_text` 升级为 `extract_tool_result_media()`：text→`model_text`、`image` 块→`follow_up_parts`（复用 read.rs 图片路径），**对今天纯文本插件是 no-op → 向后兼容**。收益：MCP 与插件**同一条路**、`list_tools` 自动上工具面（少改 run_loop/system_prompt/tool_exec）、**消除「可见走 surface/执行走前缀」不对称与 `mcp__` 抢注隐患**（路由靠注册与 `plugin_id`，非字符串前缀），并**顺带解锁「能返图的插件」**。 | 未入选：(a) surface-only + `mcp__` 前缀分支（本文初稿方案）——拒因：可见/执行两条路不对称、靠字符串前缀路由有抢注隐患、要改 run_loop/system_prompt/tool_exec 多处；(b) 注册进 registry 但**不加**转换器——拒因：图片仍回流不了，与主目标冲突。 | 让 MCP 和插件走同一条注册表路；在那条路的出口加一个「JSON 图片→图片」的小转换器——纯文本插件没图片块、完全不受影响，MCP 的截图就顺着回来了。比给 MCP 单开前缀专线更干净，还让插件将来也能返图。 |
| **R5 图片回流** | MCP 返回的 image 块怎么进模型？ | **采用** 复用 `read.rs` 的 `follow_up_parts` + `InputImage`（小图内联、大图 Files API）；**拒绝** 新建图片管道 | Tomcat：`core/agent_loop/tool_exec/branches/read.rs:319-397`、`core/llm/types.rs::ChatMessageContentPart::{image_base64_data,image_upload}`、`openai_files.rs::upload_decision_by_size`；外部：`codex/.../models.rs::convert_mcp_content_to_items`（image→input_image）、`cline::toToolResultImagePart` | 设计：base64 块 → `image_base64_data(mime,b64)`（小）或 `image_upload`/临时文件 + `OpenAiFilesRuntime`（大）；文本块→`model_text`；非 vision 由 `degrade_unsupported_multimodal` 降级。理由：零新管道，与 read 图片语义一致。 | 未入选：continue `callTool.ts` 把 image 当不支持类型报错**丢弃**；pi_agent_rust 图片入 `details` 不回流。拒因：视觉验收场景丢图 = 功能失效。 | 图片直接走「read 看图」那条现成道，别丢。 |
...
| **R8 在途失败语义** | `tools/call` 途中传输断裂怎么办？ | **采用** 只为后续调用重连、**当前调用不 replay**、有限退避 | Tomcat：`McpManager::call_tool` 错误分支（新增）；外部：`pi_agent_rust` `MCP_DELIVERY_INDETERMINATE`（不 replay）、`codex` `reinitialize_after_session_expiry`（仅 HTTP session） | 设计：stdio 传输断裂 → 当前 `call_tool` 返回明确错误，标记 server `Disconnected`，后续调用触发退避重连（≤N 次）。理由：一个可能已在 server 侧执行的调用重放会造成重复副作用（重复点击/提交）。 | 未入选：自动重放在途调用。拒因：MCP 工具可能有副作用，重放不安全。 | 断在半路的调用宁可报错，也不重来。 |
| **R9 连接时机与工具生命周期** | 配置好的 MCP server 何时连接？同步卡住首轮、后台预连，还是模型首次调用时才连？ | **采用** Chat/serve 会话初始化后，对每个**默认放行的 server**（全局/curated/已确认项目 server，R7）**后台并行预连** `ConnectorRegistry::spawn_connect_all()`（需确认的项目来源 server 不自动连、留待用户确认）；不 await、不阻塞首轮/serve handshake。`Ready` 事件才注册 `plugin_id="mcp:{server}"` 的工具；断裂/失败先 `unregister_plugin_tools`，重连 Ready 后重新登记；`McpManager` 经事件把状态交给生命周期协调器，协调器只持 `Weak<dyn ToolRegistry>`，避免强引用环。 | Tomcat：`core/skill/discovery.rs::spawn_discovery_task`、`api/chat/context.rs::spawn_skill_discovery_if_needed` 的启动期后台任务先例；`api/chat/run_loop/mod.rs::observe_tool_surface` 每轮 `list_tools(None)`，故工具登记后天然在下一轮出现。外部：Codex `codex-mcp/src/runtime.rs::McpStartupPolicy::Eager` + `connection_manager.rs::join_set.spawn`（主 agent 预连）；Continue `core/context/mcp/MCPManagerSingleton.ts::setConnections`（`void refreshConnections()`）；VS Code `mcpService.ts::autostart` / `chatServiceImpl.ts` 是可选、会等待的反例。 | **第一性原理**：模型只能调用已在本轮 ToolSurface 中的工具；若懒连到首次调用，工具尚未可见，模型没有触发连接的入口；若同步等待，慢 server/`npx` 下载把可用的首轮对话拖住。故选“后台预连 + Ready 才登记 + 每轮自动刷新”。断线必须撤销已登记工具，否则工具仍可见却必失败。Codex 的 root agent Eager、Continue IDE 异步预连支持此选择；VS Code 的可选 autostart 可在未来作为 `initial_tool_grace_ms` 增强，**本期默认 0、不延迟首轮**。 | 未入选：(a) 同步等待所有 server 后才允许首轮——拒因：任一慢/坏 server 把主交互卡住；(b) 首次工具调用才连接——拒因：未连接工具不在 ToolSurface，模型无从调用，且首个调用等待不可预测；(c) Ready 后不断线撤销——拒因：向模型暴露必失败的陈旧工具。 | 开机时悄悄并行去连；连好才让模型看见工具，断了就撤下。聊天和服务先可用，绝不等慢 MCP。 |
| **R-C1 「添加连接器」命令（两模式对等）** | 用户/GUI 怎么新增一个连接器？ | **采用** Chat 斜杠 `/connector add` + serve `add_connector`/`upsert_connector`，两模式**对等**、都落到同一份连接器写盘逻辑 | Tomcat：Chat 仿 [cmd_model.rs](../../src/api/chat/commands/cmd_model.rs)（模型 add 的现成范式）+ [commands/parse.rs](../../src/api/chat/commands/parse.rs)；serve 仿 [control.rs](../../src/api/serve/control.rs) capabilities 里 `upsert_model` 的登记 + [commands.rs](../../src/api/serve/commands.rs) 处理 + [types.rs](../../src/api/serve/types.rs)::`ServeCommand` 变体；外部：`hermes-agent hermes_cli/mcp_config.py`（`mcp add` CLI）、`vscode` mcp.json add 流 | 设计：写盘统一走 `ConnectorRegistry::upsert(connector)` → `mcp.json`（`mcpServers`，对齐 model-admin 的「唯一写盘中枢」；R10 形状）；Chat 与 serve 只是两个门面。理由：Tomcat 既有 model-admin 已是「CLI + serve 双门面 + 单写盘」的成熟范式（见 model-management-add-models.md），连接器照搬即可。 | 未入选：只做 serve 命令、Chat 不管。拒因：Chat（CLI）用户也要能加连接器，双模式对等是产品要求。 | 加连接器：命令行敲 `/connector add`、GUI 发 `add_connector`，落到同一份 `mcp.json`。 |
| **R-C2 「查看/管理连接器」命令（两模式对等）** | 怎么看/信任/删/测连接器与其工具？ | **采用** Chat `/connector list\|trust\|deny\|remove\|test` + serve `list_connectors`/`set_connector_trust`/`remove_connector`/`test_connector`，两模式对等 | Tomcat：对齐既有 `list_models`/`set_provider_key`/`remove_model`（[control.rs](../../src/api/serve/control.rs):49-53 capabilities）与 [cmd_model.rs](../../src/api/chat/commands/cmd_model.rs)；trust 落 `~/.tomcat/connector-trust.json`（R7）；外部：`hermes-agent`（`mcp list/test`）、`cline` McpHub 管理面 | 设计：`list_connectors` 返回每连接器状态 + 其工具清单 + 信任态；`set_connector_trust` 写 trust store（R7 身份校验不变）；`test_connector` 试连一次报诊断。理由：与 model-admin 的 list/key/remove 完全同构，GUI 可直接复用设置中心范式。 | 未入选：把管理揉进 add 命令。拒因：list/trust/remove/test 是不同动作，分开更清晰、可分别授权。 | 看/信任/删/测连接器：`/connector list` 或 serve `list_connectors`，和管模型一个套路。 |
...
|--------|----------------------|--------------------------|------------------|--------|
| **P0 连接器框架** | `Connector` trait + `ConnectorType {Mcp,Cli,A2a}` + `ConnectorRegistry`（按配置源构造）+ `CompositeToolExecutor`（按 `plugin_id` 路由）；`[connector] enabled` 总开关（MCP 的 `mcp.json` schema 见 P1/R10） | `core/connector/mod.rs`（trait/enum/registry/`CompositeToolExecutor`）、`infra/config/types/connector.rs`（`ConnectorConfig` 总开关）、`infra/config/types/mod.rs`（挂 `AppConfig.connector`）；预留 `core/connector/cli/`、`core/connector/a2a/`（仅目录/注释，不写实现） | `core::connector::tests::{registry_routes_by_plugin_id,connector_disabled_master_switch}` | 先把「连接器插座 + 总开关」搭好，MCP 插上去。 |
| **P1 MCP 配置与物化（`mcp.json`/`mcpServers`，R6/R10）** | `McpServerConfig`（`command`+`args` 必填、`env`/`cwd`/`trusted`/`integrity`/`startupTimeoutMs`/`callTimeoutMs`/`toolFilter` 可选）、`mcpServers` schema、user+workspace 分层覆盖、`{work_dir}/cache/playwright` 浏览器缓存目录、init 物化含 `@playwright/mcp` 的默认 | `core/connector/mcp/config.rs`（`McpServerConfig` + `serde(default)` 解析 `~/.tomcat/mcp.json` + 项目级 `.tomcat/mcp.json` 覆盖）、`core/connector/mcp/builtin.rs`（`materialize_default_mcp_json()`，仿 `skill/builtin.rs`） | `core::connector::mcp::config::tests::{parse_minimal_command_args_only,optional_fields_default,project_overrides_global,materialize_default_mcp_json_is_idempotent,playwright_entry_uses_shared_browser_cache}` | 用生态标准 `mcp.json`：只填 command/args 就能加 server，自带 playwright，浏览器共用缓存。 |
| **P2 传输与管理器** | `rmcp` stdio 传输封装、`McpManager`（连接/`list_tools`/`call_tool`）、连接状态机、超时/退避/不 replay；发 `Ready{tools}` / `NotReady` 生命周期事件 | `core/connector/mcp/transport.rs`、`core/connector/mcp/manager.rs`（`ServerState`/`ServerStatus`/`McpToolDef`/`ServerLifecycleEvent`）；`Cargo.toml` 加 `rmcp` | `core::connector::mcp::manager::tests::fake_stdio_server_lists_and_calls_tools`、`core::connector::tests::ready_mcp_tools_register_into_the_shared_tool_registry` | 把 server 拉起来、拿到工具、能调用；状态变化通知协调器挂/撤工具。 |
...
| **P5 出口媒体转换器 + 图片回流（Option B 关键）** | `tool_dispatcher` registry 分支把 `plugin_tool_model_text` 升级为 `extract_tool_result_media()`：text→`model_text`、`image` 块→`follow_up_parts`（复用 read.rs）；**对纯文本插件 no-op**；非 vision 降级 | `core/agent_loop/tool_dispatcher.rs:313-318`（registry 分支 Ok 臂，`openai_files_runtime` 在 `agent.config` 已在 scope）、复用 `branches/read.rs` 图片路径 / `types.rs::ChatMessageContentPart` / `openai_files.rs` / `multimodal.rs::degrade_unsupported_multimodal` | `core::agent_loop::tool_dispatcher::tests::{mcp_image_block_becomes_input_image,text_only_plugin_result_yields_no_follow_up_parts,non_vision_model_degrades}` | 注册表出口加个小转换器：MCP 图片翻成 InputImage，纯文本插件完全不受影响。 |
| **P6 启动装配与工具生命周期（R9）** | `ConnectorRegistry`（含 MCP 连接器）注入 `GlobalServices`；`spawn_connect_all()` 对已信任 server **后台并行预连**（非阻塞，首轮/serve handshake 不 await）；事件协调器在 Ready `register_tool`、NotReady `unregister_plugin_tools`，只持 `Weak<dyn ToolRegistry>`；skill 文档同步 | `api/chat/context.rs`（构造点，`plugin_manager` 旁，创建 `Arc<DefaultToolRegistry>` 后传入 `Weak<dyn ToolRegistry>`）、`api/chat/session_runtime.rs::GlobalServices`（加 `connector_registry` + 生命周期任务句柄）；`assets/skills/verify/SKILL.md` 更新交互路径工具清单 | `api::chat::context::tests::{connector_registry_constructed_when_enabled,startup_connect_is_non_blocking,ready_registers_tools_and_disconnect_removes_them}`、E2E `E2E-MCP-001` | 开机悄悄并行去连、不等它；连好才挂工具，断了就撤下，避免菜单里留必失败的工具。 |
| **P7 命令面（本期 CLI + serve，无 webview；R-C1/R-C2/R11）** | 「添加」`/connector add` + serve `add_connector`/`upsert_connector`；「查看/管理」`/connector list\|trust\|deny\|remove\|test` + serve 对等；**Cursor 对标补齐**：`list` 富状态（来源/连接态/工具数+资源数/信任态）、`/connector reload`（重读 `mcp.json`+重连）、`/connector tools <server>`（按 `toolFilter` 裁剪暴露工具）+ serve `reload_connector`/`set_connector_tool_filter` | `api/chat/commands/cmd_connector.rs` + `commands/parse.rs`（Chat）、`api/serve/types.rs::ServeCommand` + `commands.rs` + `control.rs`（capabilities 登记；webview 复用同一批 capabilities，本期不建 GUI）；写盘统一走 `ConnectorRegistry::upsert` → `mcp.json` | `core::connector::commands::tests::{add_then_list_roundtrips,list_reports_status_and_counts,reload_rereads_config,tools_filter_prunes_surface}`、`serve` 集成 `list_connectors_reflects_upsert`、`chat` `cmd_connector_add_and_list` | 本期命令行 + 直接改 `mcp.json`（不做界面）；命令能干 Cursor 界面能干的事：看状态、重连、裁剪工具，落到同一份配置。 |
...
> - **P9 其它连接器类型**：`core/connector/cli/`（把外部 CLI 命令包成工具）、`core/connector/a2a/`（Agent-to-Agent 协议）。框架（P0）已按 `ConnectorType` 预留路由与前缀。
...
| `args` | string[] | **是** | — | 命令行参数；npm 型**须钉死精确版本**（禁 `@latest`/`@next`/无版本，R7） | 启动参数，版本要写死。 |
...
**三个信任来源，同一道命令指纹闸（避免"两份真相"）**：放行一个 server，「我批准」可来自三处——① curated allowlist（内置默认信任）；② 用户 `/connector trust <server>`（或确认弹窗）写入 `~/.tomcat/connector-trust.json`；③ `mcp.json` 里 `"trusted": true`（声明式，供 CI/非交互）。**三者都只表达"我批准"，最终一律经同一道命令指纹匹配放行**；指纹变了（命令被换）就需重新确认。`connector-trust.json` 是确认记录，`"trusted": true` 是配置声明，两者不冲突。
...
常态（99% 情况，名字没超限）—— 模型看到的是【完整可读名】
  ...
  反向：拆前缀 → (playwright, browser_take_screenshot) → tools/call
...
单一事实源：`core/connector/mcp/naming.rs::{to_model_name, parse_model_name}`；原始 `(server, raw_tool)` 存于 `McpToolDef`，模型名仅用于暴露与分派。
...
> **`@playwright/mcp` 的截图特例（已由真实 fcodex E2E 验证）**：它只有在 `browser_take_screenshot` **省略 `filename` 且不传 `fullPage=true`** 时，才把 PNG 作为 MCP `image` block 返回；指定文件名或全页截图会只落盘、返回 Markdown 文件链接，模型看不到图。故 `assets/skills/verify/SKILL.md` 与 MCP 工具描述都明确该约束；命名/全页截图只用于人工保留文件，不能充当视觉回流证据。
...
> 专业：新增 `core/connector/` 模块——框架层（`mod.rs` trait/registry/`CompositeToolExecutor`、`config.rs`、`builtin.rs`）+ MCP 连接器 `core/connector/mcp/*`（含 `executor.rs::McpToolExecutor`）+ 命令面（Chat `cmd_connector.rs` + serve `*_connector` 三件套）+ `types/connector.rs`；预留 `cli/`、`a2a/`。**Option B（§3.1 R4）**：MCP 工具进 `ToolRegistry`、执行经 `CompositeToolExecutor` 路由、图片经 `tool_dispatcher` 出口转换器回流——核心逻辑仅改 `tool_dispatcher.rs`（转换器）与 `context.rs`（注册 + 注入 executor），**不改** `run_loop`/`system_prompt`/`tool_exec`。下图自顶向下即一次调用链路，`←新增 / 改 / 复用 / 预留` 四态标注。
> 说人话：带 `←新增` 的是全新文件，`改` 是往既有文件插一小段，`复用` 是一行都不改直接用，`预留` 是本期只留目录/注释。
...
框架层（连接器抽象）
  ...
  core/connector/cli/  core/connector/a2a/   ←预留  未来连接器类型(本期仅目录/注释)
        │
        ▼
MCP 连接器（本期实现）
  ...
  core/connector/mcp/transport.rs                ←新增  McpTransport trait + StdioTransport（封装 rmcp Transport<RoleClient>；kill_on_drop）；http 预留(P8)
  ...
  core/connector/mcp/naming.rs                   ←新增  to_model_name / parse_model_name（mcp__server__tool + 哈希）
...
启动装配 + 工具注册（Option B）
  ...
  api/chat/context.rs                  改      ConnectorRegistry 连上后 register_tool(MCP 工具, plugin_id="mcp:{server}") 进 DefaultToolRegistry；用 CompositeToolExecutor 建 registry（plugin_manager 旁）
...
工具面（每轮，无需为 MCP 改动）
  ...
  core/llm/system_prompt.rs            复用  ToolSurface::from_plugin_tools 一并纳入 MCP（不特判）
        ...
        ▼
分派 + 执行（统一走注册表 + 出口转换器）
  core/agent_loop/tool_dispatcher.rs   改      registry 分支：plugin_tool_model_text → extract_tool_result_media()
...
命令面（Chat + serve 对等，R-C1/R-C2）
  ...
  api/serve/types.rs                   改      ServeCommand: {Add,List,SetTrust,Remove,Test}Connector 变体
  api/serve/commands.rs                改      连接器命令处理（写盘走 ConnectorRegistry::upsert）
  api/serve/control.rs                 改      capabilities 登记 add_connector/list_connectors/set_connector_trust/...
配套测试
...
spawn 失败 / initialize 超时    → Failed：记录 stderr 尾部 + warning，该 server 缺席工具面（不影响其它 server 与主循环）
tools/list 分页失败            → 该 server 退化为「已连接但无工具」+ warning
tools/call 超时                → 返回明确错误文本给模型（is_error=true），不 kill server
tools/call 途中传输断裂        → 当前调用返回错误 + server 转 Disconnected（后续退避重连）；【不 replay】
image 块解码失败 / 超 IMAGE_MAX_BYTES → 跳过该图 + 文本注明「图像无法内联」；其余内容照常回流
...
| 维度 | 用例 / 编号 | 状态 | 说人话 |
|------|-------------|------|--------|
| 单元 | `core::connector::mcp::config::tests::{parse_stdio_server,project_overrides_global,materialize_default_idempotent}` | PENDING | 配置解析/覆盖/物化幂等。 |
| 单元 | `core::connector::mcp::naming::tests::{roundtrip,short_name_kept_verbatim,long_name_keeps_readable_head_plus_short_hash,mapping_table_recovers_original_after_hash,underscore_in_names}` | PENDING | 常态原样可读、超长仅截断保留可读头 + 短哈希、哈希后仍能查表反解原名。 |
| 单元 | `core::connector::mcp::trust::tests::{global_config_server_auto_trusted_no_prompt,curated_trusted_by_default,project_server_first_seen_requires_confirm,command_fingerprint_change_requires_reconfirm,user_floating_version_warns_not_blocks,optional_strong_tier_integrity_mismatch_blocks}` | PENDING | Cursor 对齐：默认放行；项目来源首见/命令变化才确认；浮动版本告警不拦（R7）。 |
| 单元 | `core::connector::mcp::manager::tests::{list_tools_after_initialize,call_tool_timeout_returns_error,transport_drop_marks_disconnected_no_replay,reconnect_refetches_tools_no_persistent_cache}` | PENDING | 连接/超时/断裂不 replay；重连现取 tools/list、不落盘缓存（R12）。 |
| 单元 | `core::agent_loop::tool_dispatcher::tests::{mcp_image_block_becomes_input_image,text_block_becomes_model_text,unknown_block_text_summary,text_only_plugin_result_yields_no_follow_up_parts,non_vision_model_degrades}` | PENDING | 出口转换器：结果分流与图片回流（**主承诺**），且纯文本插件 no-op（向后兼容）。 |
| 单元 | `core::connector::tests::mcp_tools_appear_in_surface_via_list_tools`、`core::connector::mcp::executor::tests::mcp_call_routes_to_mcp_manager` | PENDING | MCP 经 list_tools 自动上工具面；调用经 CompositeToolExecutor 落到 McpManager。 |
| 单元 | `api::chat::context::tests::{startup_connect_is_non_blocking,ready_registers_tools_and_disconnect_removes_them}` | PENDING | R9：首轮不等待 MCP；只在 Ready 挂工具、断线立即撤下，避免陈旧工具。 |
| 集成 | `tests/connector_mcp_tests.rs::fake_stdio_server_lists_and_calls`（假 stdio MCP server：返回一张图 + 一个文本工具） | PENDING | 端到端：工具进目录、调用后图片进模型消息。 |
| 集成 | `tests/connector_mcp_tests.rs::pending_confirm_project_server_absent_until_confirmed` | PENDING | 项目来源待确认 server 确认前不出现在工具面。 |
| E2E | `E2E-MCP-001`：真实 `@playwright/mcp` navigate 本地静态页 + screenshot → 断言回流（尽力，依赖 npx/Chromium） | PENDING | 真链路：截图经 MCP 回到模型。 |
| 单元 | `core::connector::tests::{registry_routes_by_plugin_id,connector_disabled_master_switch}`、`core::connector::mcp::config::tests::{parse_minimal_command_args_only,optional_fields_default,project_overrides_global,materialize_default_mcp_json_is_idempotent}` | PENDING | 框架路由/总开关 + `mcp.json`（`mcpServers`）最小字段解析/可选默认/项目覆盖/物化幂等（R-C0/R6/R10）。 |
| 单元+集成 | `core::connector::commands::tests::{add_then_list_roundtrips,list_reports_status_and_counts,reload_rereads_config,tools_filter_prunes_surface}`、serve `list_connectors_reflects_upsert`、chat `cmd_connector_add_and_list` | PENDING | 命令面 Chat+serve 对等增删查管 + Cursor 对标的富状态/重连/裁剪（R-C1/R-C2/R11）。 |
| 集成 | `tests/connector_mcp_tests.rs::pasted_cursor_snippet_connects`（把一段裸 Cursor `mcpServers` 片段丢进 `mcp.json`，仅 command/args，断言能连上并暴露工具） | PENDING | R10：生态标准片段零翻译即用。 |
| 关键承诺 | R4「MCP 进 registry + 出口转换器」→ `mcp_image_block_becomes_input_image` + `text_only_plugin_result_yields_no_follow_up_parts`（向后兼容 no-op）；R-C0「按 plugin_id 路由」→ `registry_routes_by_plugin_id`；R5「图片必须回流」→ `mcp_image_block_becomes_input_image` + `E2E-MCP-001`；R6/R10「生态标准 `mcpServers` 最小字段」→ `parse_minimal_command_args_only` + `pasted_cursor_snippet_connects`；R7「config 即信任 + 命令变化才确认（防 MCPoison）」→ `global_config_server_auto_trusted_no_prompt` + `project_server_first_seen_requires_confirm` + `command_fingerprint_change_requires_reconfirm`；R8「不 replay」→ `transport_drop_marks_disconnected_no_replay`；R9「后台预连、Ready 才挂工具、断线撤工具」→ `startup_connect_is_non_blocking` + `ready_registers_tools_and_disconnect_removes_them`；R11「CLI 对标 Cursor GUI」→ `list_reports_status_and_counts` + `reload_rereads_config` + `tools_filter_prunes_surface`；R-C1/R-C2「命令对等」→ `list_connectors_reflects_upsert` | PENDING | 决策表每条承诺都有测试锁死。 |
| 文档 | 本文定稿 + 前端 UI 验收计划 Phase 2 回链 + README 架构地图登记 | ✅ 2026-08-29 | 字与代码不两张皮。 |
...
| 子进程僵尸/泄漏 | 中 | `kill_on_drop(true)`；`McpManager::shutdown` 显式终止；进程组隔离；stderr 落 `~/.tomcat/logs/` | 关机把子进程一起带走，别留僵尸。 |
...
- ~~初稿把 MCP 配置定为自研 `connectors.toml`、每条带 `type`~~ → **改采生态标准 `mcpServers` JSON（`~/.tomcat/mcp.json`，R6/R10；用户以 Cursor 截图拍板）**：Cursor/Claude/VS Code 均用 `mcpServers`/`servers` JSON，用户从生态复制片段零翻译即用；必填收敛到 `command`+`args`（与 Cursor 一致），`type` 留代码侧（只有 MCP 时不摊派冗余字段，YAGNI）。**推翻条件**：CLI/A2A 落地、需同文件声明多类型时再评估统一 `type` 配置。连带：`ConnectorType` 抽象仍在**代码**保留（R-C0 不变），只是 MCP 的**配置文件**改用 `mcp.json`。
