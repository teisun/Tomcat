...
| E2E-CLI-096 | 自动 | `successful_boundary_runs_both_layer0_steps_invalidates_read_stamps_and_emits_once`、`all_boundary_entry_paths_inherit_layer0_cleanup`、`mid_turn_guard_boundary_runs_layer0_before_rebuilding_messages` | 任一成功 Boundary 必定紧接 L0，失败则绝不改写历史 | 分别驱动时机②（0.70 / 0.98）、时机⑤和 mid-turn guard 的已完成预热；另覆盖 stale / idle / 低水位 | 每条成功路径都有落盘 + placeholder、read stamp 失效与至多一次 release 事件；失败路径无落盘、无 placeholder |
